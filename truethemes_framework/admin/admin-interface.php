...
<div class="wrap" id="truethemes_container">
  ...
  <form action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" method="POST" enctype="multipart/form-data" id="ofform">
    ...
    <img style="display:none;" src="<?php echo get_template_directory_uri() ?>/truethemes_framework/admin/images/wpspin_light.gif" class="ajax-loading-img ajax-loading-img-bottom" alt="Working..." />
...
function of_style_only() {
	...
	if ($color == "fresh")
		{
		wp_enqueue_style('admin-style-grey', TRUETHEMES_FRAMEWORK.'/admin/admin-style-grey.css');
		wp_enqueue_style('color-picker', TRUETHEMES_FRAMEWORK.'/admin/colorpicker.css');
		}
}
...
function siteoptions_machine($options) {
	...
	foreach ($options as $value) {
		...
		/* NOT BEING USED FOR KARMA THEME
		case "typography":
