<?php
//@since version 3.0.2
//add in javascript padding fix for .content_full_width
//hook into wp_head only in jQuery 2 template and home page 3d
function tt_IE10_css_fix_for_content_full_width(){
...
add_action('wp_head','tt_IE10_css_fix_for_content_full_width');
...
$fields['author'] = '<p class="comment-input-wrap pad"><label class="comment-label" for="author">' . __( 'Name','truethemes_localize' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label>'.
...
$fields['email'] = '<p class="comment-input-wrap pad"><label class="comment-label"  for="email">' . __( 'Email','truethemes_localize' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' .
				   '<input id="email" name="email" type="text" tabindex="2" class="comment-input comment-email" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>';

//This is website field
$fields['url'] = '<p class="comment-input-wrap"><label class="comment-label" for="url">' . __( 'Website','truethemes_localize' ) . '</label>' .
		         '<input id="url" name="url" type="text" tabindex="3" class="comment-input comment-website" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p>';
...
//@since 3.0, theme check wants this, but it is not used in theme!
//so we write here and comment it out!
...
if($blog_image_frame == 'shadow'){
	$html .= '<div class="post_thumb post_shadow_thumb">';
}else{
