...
if(uds_pricing_is_plugin()) {
	...
	define('UDS_PRICING_URL', trailingslashit(get_template_directory_uri() . '/truethemes_framework/extended/pricing-tables'));
	define('UDS_PRICING_PATH', trailingslashit(get_template_directory() . '/truethemes_framework/extended/pricing-tables'));
}
		...
		if(!empty($product['uds-link']) && !empty($product['uds-link-label'])) {
			$footers .= "
				<th class='column-".($i+1)." footer $oddeven $featured_prev $featured $featured_next'>
					<a class='ka_button small_button' href='{$product['uds-link']}'><span>{$product['uds-link-label']}</span></a>
				</th>
			";
		}
